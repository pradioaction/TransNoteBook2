...
> 详细文档请参阅 [`doc/`](doc/) 目录，包含架构设计、API 接口、版本规划等。当前版本 **v2.0**。
...
│       ├── gitHubBookFetcher.ts # 远程词书获取器（GitHub/Gitee API）
│       ├── bookDAL.ts       # 词书数据访问层
│       ├── wordDAL.ts       # 单词数据访问层
...
│   │   │   ├── ImportBookDialog.tsx   # 词书导入弹窗（本地/远程）
...
- **远程词书导入** — 从 GitHub/Gitee 一键获取词书列表并导入，支持预设源和自定义仓库
...
4. 在弹出的 **导入词书** 对话框中，选择导入方式：
   - **本地导入** — 从本地文件选择器中选取 `.json` 词书文件（同 v1.4 行为）
   - **从远端获取** — 选择预设源（GitHub / Gitee）或输入自定义仓库 URL，获取词书列表后勾选导入
...
点击 **「导入词书」→「从远端获取」**，可以直接从以下预设源在线获取词书，无需手动下载：

| 源 | 地址 | 词书列表 |
|---|------|----------|
| GitHub | `KyleBing/english-vocabulary` | 初中、高中、CET4、CET6、考研、托福、SAT 等 |
| Gitee | `pradio/english-vocabulary` | 同上（国内镜像） |

也支持输入自定义仓库 URL（GitHub 或 Gitee），系统自动扫描仓库所有 `.json` 文件并按目录分组展示。

> 使用公开 API，无需 Token。如果希望导入本地已下载的 JSON 文件，选择 **「本地导入」** 即可。
...
| ▶ | 翻译当前 Cell 内容到输出区 |
| 复制图标 | 复制当前 Cell |
| ★ | 收藏当前 Cell 到指定笔记文件（需先在文件浏览器中右键设置收藏夹目标文件） |
| ✓ | AI 批阅：语法检查、用词建议、评分 |
| ↑ | 上移 Cell |
| ↓ | 下移 Cell |
| 垃圾桶 | 删除 Cell |
...
- 点击打开文件
- 右键重命名和删除
- **右键「设为收藏夹」** — 将某个文件设为单元格收藏的目标文件（以 ★ 星标图标标识）
- 目录展开/折叠
- New File 新建笔记
- Import 导入文本
- Refresh 刷新文件列表
