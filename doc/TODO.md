...
- [x] **回顾面板错词收藏**: ReviewPanel 新增"加入收藏夹"功能，可将答错单词添加到指定词书（`handleAddToFavorite` + 词书选择对话框）
- [x] **回顾面板 Bug 修复**: 修复"重新检测"按钮调用 `reset()` 导致 `active: false` 跳转到异常阅读界面的问题，改为和"返回"按钮一致的行为——保留选中词书、清空答题状态回到词书管理界面
...
- [x] **翻译进度可视化**: ✅ 已完成（底部 Panel + 单元格状态指示器 + "翻译全部"按钮）
- [x] **环境变量配置 UI 增强**: 方便添加/编辑 `ARK_API_KEY`、`OPENAI_API_KEY` 等
...
- [x] 远程词书导入：从 GitHub/Gitee 一键导入词书，支持预设源和自定义仓库 URL
- [x] 词书界面 UI 改进：RecitationShell 状态页背景色跟随主题
- [x] 统计面板 Bug 修复：无词书时不再无限加载，显示空状态提示
- [x] 统计面板自动刷新：导入词书后统计面板自动更新
- [x] 输出面板日志增强：远程导入操作添加耗时日志，便于排查网络问题
- [x] **侧边栏搜索功能**：在当前笔记中搜索文本，展示上下文片段，点击跳转到目标 cell 并高亮匹配文本
