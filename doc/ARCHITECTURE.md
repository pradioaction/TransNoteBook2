...
| 文件 | 核心类型 | 说明 |
|------|---------|------|
| `types.ts` | `Book`, `Word`, `UserStudy`, `BookProgress`, `BookWithProgress`, `TodayWordsResult`, `StudyConfig` | 数据模型 |
...
- [ ] 快捷键完善（`Ctrl+S` 保存、`Ctrl+O` 打开、`Ctrl+Enter` 翻译等）
- [ ] 翻译错误重试机制
- [ ] 翻译缓存
