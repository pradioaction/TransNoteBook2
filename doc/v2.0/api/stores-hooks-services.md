...
interface LogEntry {
  ...
  color?: string      // 自定义颜色，优先级高于 level 默认色
}
...
### 4.7 useWorkspaceConfigStore (workspaceConfigStore.ts)
...
单元格收藏 Hook，实现在 `src/hooks/useBookmark.ts`。
...
背诵服务接口，实现在 `services/recitationService.ts`。
...
interface RecitationService {
  ...
  batchImportWords(bookId: number): Promise<BatchOperationResult>   // 仅存根，待后续实现

  // === v2.0 新增 ===

  /** 获取远程仓库词书文件列表（递归扫描全仓库） */
  fetchRemoteBooks(source: { label: string; owner: string; repo: string; path: string; platform: 'github' | 'gitee' }): Promise<{ success: boolean; books: Array<{ name: string; dir: string; size: number | null; downloadUrl: string }>; error?: string }>
