...
## 14.11 远程词书导入 (Remote Book Import) — v2.0 新增

### 功能描述

用户可以从 GitHub 或 Gitee 远程仓库直接获取词书列表并导入，无需手动下载 JSON 文件。系统递归扫描整个仓库，按目录分组展示所有 `.json` 文件。

### 预设源

| 源 | 地址 |
|---|------|
| GitHub | `KyleBing/english-vocabulary` (json/) |
| Gitee | `pradio/english-vocabulary` (json/) |

无需 Token，使用公开 API 即可。

### 新增 IPC 通道

| 通道 | 说明 |
|------|------|
| `recitation:fetch-remote-books` | 获取远程仓库所有 JSON 文件列表 |
| `recitation:import-remote-book` | 下载指定 JSON 并导入到本地数据库 |

### 新增/修改文件

| 文件 | 说明 |
|------|------|
| `electron/recitation/gitHubBookFetcher.ts` | GitHub/Gitee API 封装：URL 解析、递归文件树扫描、JSON 下载 |
| `electron/recitation/bookImporter.ts` | 新增 `importFromContent(content, bookName)` 内存导入 |
| `electron/recitation/bookService.ts` | 新增 `importFromJsonContent(data, bookName)` |
| `src/components/recitation/ImportBookDialog.tsx` | 导入弹窗（本地/远端两个 tab） |
| `src/components/recitation/BookManagerPanel.tsx` | 导入按钮改为弹出 ImportBookDialog |

### 数据流

```
用户选择预设源 / 输入自定义 URL
  → 解析为 BookSource (owner/repo/path/platform)
  → fetchRemoteBooks(source)
    → GET .../git/trees/master?recursive=1 (全仓库文件树)
    → 过滤 .json 文件，按目录分组
  → 用户勾选词书
  → importRemoteBook(downloadUrl, bookName)
    → downloadJson(downloadUrl)
    → bookService.importFromJsonContent(content, name)
      → bookImporter.importFromContent(content, name)
      → 写入 SQLite (book + word 表)
  → 输出面板日志（含耗时）
```

### 输出面板日志

每个操作输出到底部 Panel，含耗时信息：

```
获取词书列表: GitHub - KyleBing/english-vocabulary (github)
获取词书列表完成: 47 个文件 (1240ms)          ← 绿色
开始导入: 3-CET4-顺序.json (1/3)
导入完成: 3-CET4-顺序.json (2150ms)           ← 绿色
导入失败: IELTS_2.json - 无法连接 (7200ms)    ← 红色
```

### UI/UX 改进 (v2.0)

- RecitationShell 状态页面背景色跟随主题
- StatsPanel 无词书时显示空状态提示，不再无限加载
- StatsPanel 词书列表在导入后自动刷新
