...
### P2-5 🟡 文章单词标注增强：NLP 词形还原
- [ ] 当前 `generateWordForms()` 仅覆盖规则变化（-s/-ing/-ed/-er/-est/-ly），无法匹配不规则变化（`ran→run`, `went→go`, `children→child` 等）
- [ ] 引入轻量 lemmatization 库，在 `processArticleText()` 中增加 NLP 词形还原步骤，将文章中的屈折形式还原为 lemma 后再匹配词库

### P2-6 🟡 阅读模式右键菜单添加查找功能
- [ ] 在 `CellEditor` 和 `CellOutput` 的阅读模式下，右键菜单增加"查找"选项，打开搜索面板或弹出查找输入框
- [ ] 与现有搜索高亮功能联动，输入关键词后高亮匹配文本

### P2-7 🟡 阅读模式进入测试时新增返回键
- [ ] 从文章阅读界面进入单词检测后，提供返回按钮/路径，让用户可以回到阅读界面而非直接跳到词书管理

### P2-8 🟡 检测后单词查阅卡片
- [ ] 检测完成后，提供单词查阅功能卡片类界面，方便用户逐一回顾答错/答对的单词
- [ ] 卡片应包含单词、释义、音标、例句等信息，支持左右滑动切换
