...
.transnb 文件的序列化格式。`wordMeta` 在 v1.3 新增，用于文章检测场景。
...
`ThemeConfig` 定义在 `src/types/notebook.ts`，包含全部 58 个颜色键。分为以下颜色组：
...
词书。可导入 KyleBing 格式 JSON 文件或手动创建。
...
10 阶段原始分布（未学 + stage 0-8），由数据层 `recitation/statDAL.ts` 统计。
...
今日单词结果，含新学单词和复习单词列表，以及已测单词 ID 列表。
...
定义于 `src/store/settingStore.ts`。所有 `set*` 方法均使用 500ms debounced 持久化，避免频繁写盘。通过 `_onThemeChange` 回调解耦 themeStore 依赖（settingStore 在 `saveToDisk` 时读取 themeStore 的当前主题，而非直接引用 themeStore 的方法）。
