...
interface NotebookData {
  ...
  wordMeta?: ArticleWordMeta   // 文章关联词书单词元数据（背诵模式）
}
...
以下类型定义在 `src/types/electron.ts`（v1.2 新增：统一共享类型定义源）。
...
以下类型定义在 `src/recitation/types.ts`。
...
> 在 `src/recitation/quizTypes.ts` 和 `src/recitation/wordSidebarTypes.ts` 中还有 `QuizState`、`QuizQuestion`、`QuizOption`（含 `pairText` 字段支持悬停切换显示）、`WordSidebarData` 等 UI 相关的背诵模式类型。
...
背诵模式 UI 状态 Store，管理背诵模式的阶段切换、侧边栏数据和检测状态。
...
interface RecitationStore {
  ...
  setSidebarMode(mode: 'full' | 'quiz' | 'review'): void
...
- 以下常见快捷键待添加（见 TODO.md 第 3 节）：`Ctrl+S` 保存、`Ctrl+Shift+S` 另存为、`Ctrl+O` 打开、`Ctrl+Enter` 翻译、`Ctrl+B` 切换侧边栏。
...
```typescript
const Component = () => {
  const recitation = useRecitationService()

  useEffect(() => {
    recitation.init('/path/to/workspace')
    recitation.getAllBooksWithProgress().then(books => {
      console.log('词书列表:', books)
    })
  }, [])

  const handleStudy = async (bookId: number) => {
    const result = await recitation.getTodayWords(bookId)
    console.log('今日新词:', result.newWords)
    console.log('今日复习:', result.reviewWords)
  }
}
```
...
```typescript
import type { FileService, CellService, TranslationService, TranslationStatus, ProviderInfo, RecitationService } from '@/services/types'

// 注意: v1.2 移除了 services/fileService.ts 和 services/cellService.ts 的死代码。
// 服务层仅保留 types.ts (类型定义) 和 translationService.ts / recitationService.ts (有状态实现)。
// FileService 和 CellService 的实现直接位于 hooks/useFileService.ts 和 hooks/useCellService.ts 中。
```
...
**实现**: 服务层逻辑直接实现在 `useFileService()` Hook 中（模块级闭包）
**依赖**: notebookStore, workspaceStore, window.electronAPI
**位置**: `src/hooks/useFileService.ts`（v1.2 移除了 `src/services/fileService.ts` 死代码）
...
**实现**: 服务层逻辑直接实现在 `useCellService()` Hook 中（模块级闭包）
**依赖**: notebookStore
**位置**: `src/hooks/useCellService.ts`（v1.2 移除了 `src/services/cellService.ts` 死代码）
...
interface TranslationStatus {
  ...
  currentContent?: string    // 当前翻译中的内容预览
}
...
**实现**: `createRecitationService()` (服务层) / `useRecitationService()` (React Hook)
**依赖**: window.electronAPI.recitationAPI
