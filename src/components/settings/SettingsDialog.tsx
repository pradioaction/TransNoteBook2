    ...
    >
      ...
      >
        ...
        <div style={{ display: 'flex', borderBottom: `1px solid ${colors.border}`, paddingLeft: 8 }}>
          ...
          <button style={tabStyle(activeTab === 'translation')} onClick={() => setActiveTab('translation')}>Translation</button>
          <button style={tabStyle(activeTab === 'templates')} onClick={() => setActiveTab('templates')}>Prompts</button>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: 16 }}>
          ...
          {activeTab === 'general' && (
            <div>
              ...
              <div style={{ marginBottom: 16 }}>
                <label style={labelStyle}>Reading Font Size</label>
              ...
              <div style={{ marginBottom: 16 }}>
                <label style={labelStyle}>单元格宽度比例</label>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <input
                    type="range"
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                {settingStore.envVars.length === 0 && (
                  <div style={{ fontSize: 12, color: '#999', fontStyle: 'italic', marginBottom: 8 }}>No env vars configured</div>
                )}
                ...
                <div style={{ display: 'flex', gap: 8, marginTop: 8, alignItems: 'center' }}>
                  ...
                  />
                  <button onClick={handleAddEnvVar} style={primaryBtn}>Add</button>
                </div>
              </div>
            </div>
          )}
            ...
            <div>
              ...
              <div style={{ marginBottom: 12 }}>
                ...
                <select
                  ...
                  style={inputStyle}
                >
                  <option value="system_Ollama">Ollama (Local)</option>
                  <option value="system_OpenAI">OpenAI Compatible</option>
                  {settingStore.customModels.map((m) => (
                    <option key={m.name} value={`custom_${m.name}`}>Custom: {m.name}</option>
                  ))}
                </select>
              </div>

              <div style={{ border: `1px solid ${colors.border}`, borderRadius: 4, padding: 12, marginBottom: 12 }}>
                <div style={{ fontSize: 12, fontWeight: 600, color: colors.foreground, marginBottom: 8 }}>Ollama</div>
                ...
                <div style={{ marginTop: 8 }}>
                  ...
                  >
                    {testingProvider === 'system_Ollama' ? 'Testing...' : 'Test Connection'}
                  </button>
              ...
              <div style={{ border: `1px solid ${colors.border}`, borderRadius: 4, padding: 12, marginBottom: 12 }}>
                <div style={{ fontSize: 12, fontWeight: 600, color: colors.foreground, marginBottom: 8 }}>OpenAI Compatible</div>
                <div style={{ marginBottom: 8 }}>
                  <label style={labelStyle}>Base URL</label>
                ...
                <div style={{ marginTop: 8 }}>
                  <button
                    ...
                    style={{ ...btnStyle, border: `1px solid ${colors.border}`, color: colors.foreground }}
                  >
                    {testingProvider === 'system_OpenAI' ? 'Testing...' : 'Test Connection'}
                  </button>
          ...
          {activeTab === 'templates' && (
            <div>
              <div style={{ marginBottom: 12 }}>
                <label style={labelStyle}>Translation Prompt</label>
                ...
                <div style={{ fontSize: 11, color: '#999', marginTop: 2 }}>Use {'{input}'} as placeholder for text</div>
              </div>
              <div style={{ marginBottom: 12 }}>
                <label style={labelStyle}>Analysis Prompt (used in cells)</label>
              ...
              <div style={{ marginBottom: 12 }}>
                <label style={labelStyle}>Scene/Article Generation Prompt</label>
            ...
            <div>
              <div style={{ marginBottom: 12, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontSize: 12, fontWeight: 600, color: colors.foreground }}>
                  Custom Models ({settingStore.customModels.length})
                </span>
                <button onClick={() => setNewModelOpen(true)} style={primaryBtn}>+ Add Model</button>
              </div>

              {settingStore.customModels.length === 0 && (
                <div style={{ fontSize: 12, color: '#999', fontStyle: 'italic' }}>No custom models configured</div>
              )}
                ...
                <div key={m.name} style={{ border: `1px solid ${colors.border}`, borderRadius: 4, padding: 10, marginBottom: 8 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                    ...
                    <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                      <button onClick={() => handleEditModel(m)} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 11, color: colors.primaryButton }}>Edit</button>
                      <button
                      ...
                      >
                        {testingProvider === `custom_${m.name}` ? 'Testing...' : 'Test'}
                      </button>
                      {renderTestResult(`custom_${m.name}`)}
                      <button onClick={() => handleRemoveModel(m.name)} style={{ background: 'none', border: 'none', color: '#e06c75', cursor: 'pointer', fontSize: 12 }}>Remove</button>
                    </div>
                  </div>
              ...
              {newModelOpen && (
                <div style={{ border: `1px solid ${colors.primaryButton}`, borderRadius: 4, padding: 12, marginTop: 8 }}>
                  <div style={{ marginBottom: 8, fontSize: 12, fontWeight: 600, color: colors.foreground }}>
                    {editingModelName ? 'Edit Model' : 'Add Model'}
                  </div>
                  <div style={{ marginBottom: 8 }}>
                  ...
                  <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
                    <button onClick={() => setNewModelOpen(false)} style={{ ...btnStyle, border: `1px solid ${colors.border}`, color: colors.foreground }}>Cancel</button>
                    <button onClick={handleAddModel} style={primaryBtn}>{editingModelName ? 'Save' : 'Add'}</button>
                  </div>
                </div>
              )}
            </div>
