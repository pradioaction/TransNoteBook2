...
`generateSceneText(words, promptTemplate?)` 是 TranslationService 的预留接口，供未来的背诵模块生成场景文章使用。当前通过当前翻译提供者调用，提示词模板来自 `settingStore.promptTemplates.scenery`。
...
5. **背诵模式** (Not started): 对标原项目的完整单词学习系统（TranslationService 已预留 `generateSceneText` 接口）
